...
export async function getFeaturedPhotos(imgDimensions=false) {
  ...
  for (const image of photos.images) {
    photos.img.push({
      src: urlFor(image).url(),
      ...(await imgPlaceholderAttr(
        urlFor(image).width(10).url(),
        { dimensions: imgDimensions, w:400 }
      )),
    });
  }
...
export async function getAllCommunities({imgDimensions=false}) {
  ...
  for (const community of communities) {
    ...
    if (community.poster) {
      img.src = urlFor(community.poster).width(600).height(600).fit("max").url();
      img.overlay_src = urlFor(community.poster).fit("max").url();
    } else {
      img.src = PLACEHOLDER_IMAGE_URL;
      img.overlay_src= PLACEHOLDER_IMAGE_URL;
    }
    ...
    community.img = {
      ...
      ...await imgPlaceholderAttr(
        community.poster
        ? urlFor(community.poster)?.width(10).height(10).fit("max").url()
