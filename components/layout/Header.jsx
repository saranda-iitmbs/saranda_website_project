...
export default function Header({className = ""}) {
  ...
  const navbar = <nav className={`
    text-center max-md:z-21 max-md:w-fit
    ${sidebarOpend ? "max-md:block": "max-md:hidden"}
  `} onClick={e => setSidebarOpened(false)}>
    <NavButton href="/">Home</NavButton>
    <NavButton href="/know_us">Know Us</NavButton>
    <NavButton href="/events">Events</NavButton>
    <NavButton href="/meetups">Meet Ups</NavButton>
    <NavButton href="/art_gallery">Art Gallery</NavButton>
    <DropDown
...
function NavButton({
  ...
  return (
    ...
    ` + (path == href ? " border-b-2 max-md:border-t-2" : "")}>
      <div className={`
