...
export default function Team({ team, className = "", ...props }) {
  ...
  // Prevent crash if Sanity returns null
  if (!team) {
    return null;
  }
      ...
      <div className="relative bg-gradient-to-br from-[#1a4d2e]/85 via-[#2d5016]/80 to-[#1a3825]/90 backdrop-blur-xl rounded-2xl md:rounded-3xl overflow-hidden border border-[#2d5016]/50 shadow-[0_18px_50px_rgba(0,0,0,0.5),0_0_40px_rgba(26,77,46,0.25)] md:shadow-[0_30px_80px_rgba(0,0,0,0.65),0_0_60px_rgba(26,77,46,0.28)]">
        ...
        >
          <div className="inline-block relative">
            ...
            <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[#f8e41a] drop-shadow-[0_0_8px_rgba(248,228,26,0.35)]">
              ...
              >
                <path
                  d="M6 22c18-10 36-12 54-4s36 6 54-4"
                  stroke="#f8e41a"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeOpacity="0.7"
        ...
        >
          {members.map((member, index) => (
            <Member member={member} key={member?._key || index} index={index} />
          ))}
        </div>
  ...
  return (
    <div
      ...
      className={twJoin(
        ...
        "shadow-[0_8px_25px_rgba(0,0,0,0.35),0_0_0_1px_rgba(248,228,26,0.08)] md:shadow-[0_12px_35px_rgba(0,0,0,0.4),0_0_0_1px_rgba(248,228,26,0.08)]",

        "hover:shadow-[0_20px_40px_rgba(0,0,0,0.6),0_0_20px_rgba(248,228,26,0.25)] md:hover:shadow-[0_25px_50px_rgba(0,0,0,0.6),0_0_25px_rgba(248,228,26,0.25)]",
        ...
        <div className="pt-2 md:pt-3 flex justify-center gap-1.5 md:gap-2 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-1 group-hover:translate-y-0">
          ...
          {Array.isArray(member.socials) &&
            member.socials.map((social) => {
              ...
              const { socialMedia, url, _key } = social;

              // Ignore old/incomplete Sanity data
              if (!socialMedia || !url) {
                return null;
              }

              const Icon = socialMediaIcons[socialMedia];
...
function SocialLinkIcon({ Icon, href, ...props }) {
  // CRITICAL:
  // Never allow null/undefined/empty href
  // to reach Next.js <Link>.

  if (!href || typeof href !== "string" || !Icon) {
    return null;
  }
    ...
    <>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          ...
          style={{
            background: `radial-gradient(circle at 50% 50%, ${theme.base}55, transparent 65%)`,
          }}
        />
        ...
        <div
          ...
          style={{
            background: `radial-gradient(circle at 50% 50%, ${theme.moss}50, transparent 70%)`,
          }}
        />
      </div>
